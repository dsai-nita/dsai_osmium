...
import { Play, Trophy, Star, Clock, CheckCircle, XCircle, RotateCcw, Code, Brain, Gamepad2, Target, Award, Zap, Cpu, Activity, MessageCircle, Camera, Loader2 } from 'lucide-react'
...
import { quizzesApi } from '../lib/endpoints'
import { adaptQuizSummary, adaptQuizDetail } from '../lib/adapters'
...
const QUIZ_ICON_MAP: Record<string, any> = {
  Brain, Gamepad2, Code, Target, Zap, Cpu, Activity, MessageCircle, Camera, Award,
}
const getQuizIcon = (key: string) => QUIZ_ICON_MAP[key] || Brain
...
export function QuizPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await quizzesApi.list({ limit: 100 })
        if (!cancelled) setQuizList((res.data || []).map(adaptQuizSummary))
      } catch {
        if (!cancelled) setQuizList([])
      } finally {
  ...
  const startQuiz = async (quizSummary: any) => {
    if (!isLoggedIn) {
      toast.error('Please log in to take a quiz', {
        description: 'Your score is tracked against your account for the leaderboard.',
      })
      return
    }
    ...
    try {
      const res = await quizzesApi.get(quizSummary.id)
      const quiz = adaptQuizDetail(res.data)
  ...
  const handleQuizComplete = async (answers = userAnswers) => {
    ...
    const elapsed = selectedQuiz.timeLimit * 60 - timeLeft
    const payloadAnswers = selectedQuiz.questions.map((q, index) => ({
      questionId: q.id,
      selectedOption: answers[index],
    })).filter((a) => a.selectedOption !== undefined)

    try {
      const res = await quizzesApi.submit(selectedQuiz.id, {
        answers: payloadAnswers,
        timeTaken: Math.max(elapsed, 0),
      })
      setScore(res.data.score)
    } catch (err: any) {
      toast.error('Could not submit quiz', { description: err?.message })
      setScore(0)
    }
          ...
          <div className="max-w-6xl mx-auto text-center">
            ...
            >
              <div className="bg-card border border-border rounded-lg p-6 glow-accent">
                ...
                <p className="text-2xl font-bold gradient-text">{quizList.length}</p>
              </div>
              <div className="bg-card border border-border rounded-lg p-6 glow-cyan">
                ...
                <p className="text-2xl font-bold gradient-text">
                  {quizList.reduce((total, quiz) => total + (Number(quiz.questionCount) || 0), 0)}
                </p>
              </div>
              <div className="bg-card border border-border rounded-lg p-6 glow-primary">
                ...
                <p className="text-2xl font-bold gradient-text">
                 {quizList.reduce((total, quiz) => total + (Number(quiz.totalPoints) || 0), 0)}
                </p>
              </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {quizList.map((quiz, index) => {
                const Icon = getQuizIcon(quiz.iconKey)
                ...
                const colorClass = categoryColors[quiz.category] || 'bg-muted text-muted-foreground border-border'
                ...
                return (
                  ...
                  >
                    <Card className="p-6 h-full hover:border-accent transition-all duration-300 group hover:glow-accent cursor-pointer">
                      ...
                      >
                        {isLoadingQuiz ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Play className="h-4 w-4 mr-2" />}
                        Start Quiz
                      </Button>
                    </Card>
                  </motion.div>
...
const totalPoints = Number(selectedQuiz?.totalPoints) || 0;

const percentage =
  totalPoints > 0
    ? Math.round((scoreNum / totalPoints) * 100)
    : 0;
